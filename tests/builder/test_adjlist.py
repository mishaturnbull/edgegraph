...
def test_adjlist_edgetype():
    ...
    assert isinstance(l0to1, DirectedEdge), "load_adj_dict used wrong edge type!"
    assert isinstance(l1to2, DirectedEdge), "load_adj_dict used wrong edge type!"
...
def test_adjlist_build_clrs09_22_1():
    ...
    assert v[0].links[1].other(v[0]) is v[4], "v0 -- v4 link is not right!"

    # v1 links
    assert len(v[1].links) == 4, "v1 has wrong # of links!"
    assert v[1].links[0].other(v[1]) is v[0], "v1 -- v0 (back) link is not right!"
    assert v[1].links[1].other(v[1]) is v[2], "v1 -- v2 link is not right!"
    assert v[1].links[2].other(v[1]) is v[3], "v1 -- v3 link is not right!"
    assert v[1].links[3].other(v[1]) is v[4], "v1 -- v4 link is not right!"

    # v2 links
    assert len(v[2].links) == 2, "v2 has wrong # of links!"
    assert v[2].links[0].other(v[2]) is v[1], "v2 -- v1 (back) link is not right!"
    assert v[2].links[1].other(v[2]) is v[3], "v2 -- v3 link is not right!"

    # v3 links
    assert len(v[3].links) == 3, "v3 has wrong # of links!"
    assert v[3].links[0].other(v[3]) is v[1], "v3 -- v1 (back) link is not right!"
    assert v[3].links[1].other(v[3]) is v[2], "v3 -- v2 (back) link is not right!"
    assert v[3].links[2].other(v[3]) is v[4], "v3 -- v4 link is not right!"

    # v4 links
    assert len(v[4].links) == 3, "v4 has wrong # of links!"
    assert v[4].links[0].other(v[4]) is v[0], "v4 -- v0 (back) link is not right!"
    assert v[4].links[1].other(v[4]) is v[1], "v4 -- v1 (back) link is not right!"
    assert v[4].links[2].other(v[4]) is v[3], "v4 -- v3 (back) link is not right!"
