...
def test_diredge_assoc_postinit():
    ...
    assert v1.links == (e,), \
            "DIrectedEdge did not bind vertex link!"
    ...
    assert e.vertices == (v1, v2), \
            "DirectedEdge did not associate v2 postinit!"
    assert v1.links == (e,), \
            "DirectedEdge v2-set altered v1 links!"
    assert v2.links == (e,), \
...
def test_diredge_assoc_update_v2():
    ...
    assert e.vertices == (v1, v3), \
            "DirEdge did not set v2 correctly!"
    assert v1.links == (e,), \
            "DirEdge v2-set altered v1 links!"
    assert v2.links == tuple(), \
            "DirEdge v2-set did not remove links from old v2!"
    assert v3.links == (e,), \
            "DirEdge v2-set did not bind to new v2!"
